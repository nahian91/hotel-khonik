...
add_shortcode('ahbn_my_account', function(){
    ...
    if(!is_user_logged_in()){
        echo '<p>Please <a href="' . wp_login_url(get_permalink()) . '">login</a> to access your account.</p>';
        return '';
    }
    ...
    if(isset($_POST['ahbn_save_profile'])){
        if(!wp_verify_nonce($_POST['ahbn_profile_nonce'] ?? '', 'ahbn_profile_save')) {
            echo '<div class="ahbn-notice ahbn-error">Security check failed!</div>';
        } else {
            // Name
            $name = sanitize_text_field($_POST['ahbn_name']);
            wp_update_user(['ID'=>$user_id,'display_name'=>$name,'user_nicename'=>sanitize_title($name)]);

            // Password
            if(!empty($_POST['ahbn_password'])){
                if($_POST['ahbn_password'] === $_POST['ahbn_password_confirm']){
                    wp_set_password(sanitize_text_field($_POST['ahbn_password']), $user_id);
                } else {
                    echo '<div class="ahbn-notice ahbn-error">Passwords do not match!</div>';
                }
            }

            // Images
            update_user_meta($user_id,'ahbn_profile_image',intval($_POST['ahbn_profile_image']));
            update_user_meta($user_id,'ahbn_profile_nid',intval($_POST['ahbn_profile_nid']));

            echo '<div class="ahbn-notice ahbn-success">Profile saved successfully!</div>';
        }
    }
        ...
        <div class="ahbn-tab-content active" id="tab-profile">
            <form method="post">
                ...
                <p>
                    ...
                    <button id="ahbn_profile_image_button">Select/Upload Image</button>
                    <button id="ahbn_profile_image_remove">Remove</button>
                </p>

                <!-- NID Image -->
                <p>
                    <label>NID / Passport</label><br>
                    ...
                    <button id="ahbn_profile_nid_button">Select/Upload NID</button>
                    <button id="ahbn_profile_nid_remove">Remove</button>
                </p>

                <p><input type="submit" name="ahbn_save_profile" value="Save Profile"></p>
            </form>
        </div>
